...
export default function AppNavMenu({ isOpen, onClose }: AppNavMenuProps) {
  ...
  return (
    <>
      ...
      {/* Menu Drawer - Overlay transparent */}
      <aside
        className={`
          fixed inset-y-0 left-0 z-[70]
          bg-background-overlay/95 backdrop-blur-xl border-r border-borders-subtle/50
          w-52 transform transition-transform duration-300 ease-in-out shadow-2xl
          ${isOpen ? 'translate-x-0' : '-translate-x-full'}
        `}
      >
        {/* Header */}
        <div className="h-14 flex items-center justify-between px-3 border-b border-borders-subtle">
          <h2 className="text-base font-semibold text-text-primary">Menu</h2>
          <button
            onClick={onClose}
            className="p-1.5 text-text-secondary hover:text-text-primary hover:bg-background-glass-subtle rounded-lg transition-colors"
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-3 py-1.5 mx-1.5 mb-0.5 rounded-lg transition-colors ${
                  isActive
              ...
              >
                ...
                {item.path === '/alerts' && unreadAlertsCount > 0 && (
                  <span className="px-1.5 py-0.5 text-xs font-semibold bg-[#E1463E] text-white rounded-full min-w-[18px] text-center">
