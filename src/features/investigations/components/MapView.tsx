/**
 * Map View — grid of nodes; optional context layer for selected path.
 */
  ...
  if (nodes.length === 0) {
    return (
      <div className={`rounded-xl border border-borders-subtle bg-background-base p-6 text-center text-text-muted text-sm ${className}`}>
        Aucun nœud. Lancez la collecte pour construire la carte.
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-xl border border-borders-subtle bg-background-base overflow-hidden ${className}`}>
      <div className="p-3 border-b border-borders-subtle">
        <h2 className="text-sm font-semibold text-text-primary">Carte</h2>
        <p className="text-xs text-text-muted mt-0.5">Contexte spatial (optionnel). Cliquez sur un nœud.</p>
      ...
      <div className="p-4 overflow-auto">
        ...
        >
          {edges.map((e, i) => {
            ...
            return (
              <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#6b7280" strokeWidth={1} strokeOpacity={0.6} />
            );
          })}
          {nodes.map((node, i) => {
            const col = i % cols;
            ...
            return (
              <g key={node.id}>
                ...
                />
                <text x={cx} y={cy + 4} textAnchor="middle" fontSize="10" fill="#e5e7eb" className="pointer-events-none">
