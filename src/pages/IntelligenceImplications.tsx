/**
 * Intelligence Implications Page
 * 
 * Dedicated page for Key Implications analysis
 * Deep dive into high-certainty implications and their cascading effects
 */

import { useState, useMemo } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { ArrowLeft, CheckCircle2, TrendingUp, Filter, Link2, Search as SearchIcon, ExternalLink } from 'lucide-react';
...
export default function IntelligenceImplications() {
  return (
    <ProtectedRoute>
      <IntelligenceImplicationsContent />
    </ProtectedRoute>
  );
}

function IntelligenceImplicationsContent() {
  const location = useLocation();
  const navigate = useNavigate();
  const state = location.state as LocationState | null;
  
  const [sortBy, setSortBy] = useState<'certainty' | 'sector' | 'actor'>('certainty');
  ...
  const allClaims = useMemo(() => {
    if (state?.claims) {
      return state.claims.filter(c => c.type === 'implication' && c.certainty >= 0.6);
    }
    return [];
  }, [state]);
  ...
  const sectors = useMemo(() => {
    ...
    allClaims.forEach(c => {
      if (c.sectors) {
        c.sectors.forEach(s => sectorSet.add(s));
      }
    });
  ...
  const filteredClaims = useMemo(() => {
    let filtered = allClaims.filter(c => {
      ...
      if (filterSector !== 'all' && (!c.sectors || !c.sectors.includes(filterSector))) return false;
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'sector':
          const aSector = a.sectors?.[0] || '';
          const bSector = b.sectors?.[0] || '';
    ...
    return filtered;
  }, [allClaims, sortBy, filterCertainty, filterSector, searchQuery, filterEntity, filterRegion]);
  ...
  return (
    <AppShell>
      <SEO 
        title="Key Implications — Intelligence | Nucigen Labs"
        description="Detailed analysis of high-certainty implications and cascading effects"
      />
      ...
      <div className="col-span-1 sm:col-span-12">
        ...
        <div className="mb-6">
          ...
          <SectionHeader
            ...
            subtitle={`${allClaims.length} high-certainty implications identified`}
          />
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-text-tertiary font-light mb-1">Total Implications</p>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-semibold text-purple-500">{(avgCertainty * 100).toFixed(0)}%</p>
                </div>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-text-tertiary font-light mb-1">Affected Sectors</p>
                  <p className="text-2xl font-semibold text-text-primary">{sectors.length}</p>
                </div>
        ...
        {Object.keys(bySector).length > 0 && (
          <Card className="p-4 mb-6">
            <p className="text-xs text-text-tertiary font-light mb-3 uppercase tracking-wider">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-light border transition-colors ${
                      filterSector === sector
                        ...
                        : 'bg-white/[0.02] text-text-secondary border-white/[0.08] hover:bg-white/[0.05]'
                    }`}
                  >
                    {sector} ({count})
                  </button>
        ...
        {filteredClaims.length === 0 ? (
          <Card className="p-12 text-center">
            <CheckCircle2 className="w-16 h-16 text-text-tertiary mx-auto mb-4 opacity-50" />
            <h3 className="text-lg text-text-primary font-light mb-2">No implications found</h3>
            <p className="text-sm text-text-secondary font-light">
              {allClaims.length === 0
                ? 'No high-certainty implications identified in this search.'
                : 'Try adjusting your filters to see more implications.'}
            </p>
          </Card>
        ) : (
            ...
            {filteredClaims.map((claim) => {
              ...
              return (
                <Card 
                  key={claim.id} 
                  id={`claim-${claim.id}`}
                  className="p-6 border-purple-500/20 bg-purple-500/5 hover:bg-purple-500/10 transition-colors"
                >
                          ...
                          <>
                            {claim.sectors.map((sector, idx) => (
                              <button
                                key={idx}
                                onClick={() => {
                                  setFilterSector(filterSector === sector ? 'all' : sector);
                                }}
                        ...
                        <div key={idx} className="pl-4 border-l-2 border-purple-500/30">
                          ...
                          {typeof evidence !== 'string' && evidence.type === 'article' && evidence.url && (
                            <a
                              ...
                              className="flex items-center gap-2 text-xs text-purple-400 hover:text-purple-300 transition-colors mt-1"
                            >
